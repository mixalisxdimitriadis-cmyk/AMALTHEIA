...
// ΚΥΚΛΙΚΗ ΟΜΑΔΑ – INTERACTIVE

document.addEventListener('DOMContentLoaded', function() {
    const wheelMembers = document.querySelectorAll('.wheel-member');
    ...
    const steeringWheel = document.querySelector('.steering-wheel-img');
    
    // Κρατάμε το τρέχον active member
    let currentActive = null;
    
    // Αλλαγή κεντρικής φωτογραφίας με animation
    function updateCenter(member) {
        ...
        setTimeout(() => {
            ...
            // Ελαφριά περιστροφή του τιμονιού (προαιρετικό)
            steeringWheel.style.transform = `rotate(${Math.random() * 10 - 5}deg)`;
            steeringWheel.style.transition = 'transform 0.5s ease';
    ...
    // Ενεργοποίηση μέλους
    function activateMember(clickedMember) {
        // Αν είναι ήδη active, μην κάνεις τίποτα
        if (clickedMember.classList.contains('active')) return;
        
        // Αφαίρεση active class από όλα
        wheelMembers.forEach(item => {
            item.classList.remove('active');
        });
        
        // Προσθήκη active class στο νέο
        clickedMember.classList.add('active');
        
        // Ενημέρωση κεντρικής φωτογραφίας
        updateCenter(clickedMember);
        
        // Αποθήκευση τρέχοντος active
        currentActive = clickedMember;
    }
    
    // Προσθήκη click event listeners
    wheelMembers.forEach(member => {
    ...
    // Κλικ έξω από τα bullets κρύβει τα κείμενα
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.wheel-member') && !e.target.closest('.center-photo-hole')) {
            wheelMembers.forEach(item => {
                item.classList.remove('active');
            });
            currentActive = null;
    ...
    // Ενεργοποίηση πρώτου μέλους by default
    if (wheelMembers.length > 0) {
        setTimeout(() => {
            activateMember(wheelMembers[0]);
        }, 800);
    }
    
    // Keyboard navigation για ευκολία
    document.addEventListener('keydown', function(e) {
        if (!currentActive) return;
        
        const activeIndex = Array.from(wheelMembers).findIndex(m => m.classList.contains('active'));
        
        if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
            e.preventDefault();
            const nextIndex = (activeIndex + 1) % wheelMembers.length;
            activateMember(wheelMembers[nextIndex]);
        } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
            e.preventDefault();
            const prevIndex = (activeIndex - 1 + wheelMembers.length) % wheelMembers.length;
            activateMember(wheelMembers[prevIndex]);
        }
    });
});
...
})(jQuery); // Αυτό είναι το κλείσιμο του αρχείου σου (γραμμή 604)